-- Altıncı tur inceleme, açık madde (aynı kartın küçük ikinci maddesi):
-- record_sales_outcome'ın won->won "düzeltme" yolu (bir lead zaten
-- KAZANILDI olarak kayıtlıyken pv_sales/pv_admin farklı bir revizyonu
-- seçip yeniden kaydettiğinde) önceki kabul edilmiş revizyonun durumunu
-- hiç geri almıyordu — iki offer_versions satırı aynı anda 'accepted'
-- görünebiliyordu (bayat bir rozet, offers.status/sales_outcomes doğru
-- kalıyor). Artık eski seçim yeni seçimden farklıysa 'superseded'e
-- dönüyor.
