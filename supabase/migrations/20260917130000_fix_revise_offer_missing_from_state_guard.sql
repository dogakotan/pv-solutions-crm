...
-- durumuna hiç bakmadan onu 'superseded' yapıp yeni bir 'sent' revizyon
-- ekliyordu. respond_to_offer'ın aksine (yalnızca status='sent' bir
-- revizyonu yanıtlamaya izin veriyor) burada hiçbir FROM-state koruması
-- yoktu. Sonuç: bir lead KAZANILDI olarak işaretlenmiş (offers.status=
-- 'accepted', en son revizyon status='accepted') olsa bile, pv_sales/
-- pv_admin lead sayfasındaki "Revize Et" butonuna basarak (buton hiçbir
-- offer/lead durumuna göre gizlenmiyor/pasifleştirilmiyor) kabul edilmiş
-- kanıtı sessizce 'superseded'e çevirip altına kararsız yeni bir revizyon
-- ekleyebiliyordu — offers.status ('accepted') ile gerçek revizyon durumu
-- arasında ve nihayetinde sales_outcomes.outcome ile de çelişen bir hâl
-- ortaya çıkabiliyordu. Düzeltme: en son revizyon zaten 'accepted' ise
-- veya offer zaten 'accepted'/'closed' (kesinleşmiş) ise revize reddedilir.
-- Gerçek SQL rol-taklidiyle doğrulandı: her iki koruma da doğru tetikleniyor,
-- 'rejected' -> 'open' yeniden açma akışı (üçüncü tur) bozulmadı.
